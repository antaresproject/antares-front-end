...
const AntaresDatatablesFilters = {
  ...
  checkCountSwiperSliders() {
    // if ($('.swiper-filters--box .swiper-slide').length > 3) {
    //     $('.swiper-filters--box .card-filters--swiper').css('justify-content', 'flex-start');
    // } else {
    //     $('.swiper-filters--box .card-filters--swiper').css('justify-content', 'flex-end');
    // }
  },
  filtersSwiper() {
    ...
    function activateSwiper() {
      ...
      function countSlidersSwiper() {
        //#226 need correct
        function updateCountSwiperSliders(slidesOnPage) {
          var swiperWidth = $('.swiper-filters').width();
          var slideWidth = $('.swiper-slide .drop-target').width() + 40;
          var widthFromSlides = slideWidth * slidesOnPage;
          if (swiperWidth > widthFromSlides) {
            if (slideWidth < swiperWidth - widthFromSlides) {
              // if swiper have free space
              slidesOnPage = slidesOnPage + 1;
            }
          } else {
            slidesOnPage = slidesOnPage - 1;
          }
        ...
        enquire.register('screen and (min-width: 768px) and (max-width: 1350px)', {
          //mobile readonly for multiple
          match: function() {
            slidesOnPage = 5;
            mySwiper.params.slidesPerView = slidesOnPage;
        ...
        enquire.register('screen and (min-width: 1351px) and (max-width: 1499px)', {
          //mobile readonly for multiple
          match: function() {
            var gridWidth = $('.swiper-filters--box').closest('.grid-stack-item').attr('data-gs-width');
            if (gridWidth <= 20) {
              slidesOnPage = 5;
            } else {
              slidesOnPage = 3;
        ...
        enquire.register('screen and (min-width: 300px)', {
          //mobile readonly for multiple
          match: function() {
            ...
            match: function() {
              setTimeout(function() {
                $('.drop-target').on('click', function() {
                  console.log();
                  var leftForDropArrow = $(this).offset().left + $(this).width() / 2 - 16;
                  var topForDrop = $(this).offset().top - $(window).scrollTop() + 30;
                  $('.drop-element').removeClass('drop-arrow--up drop-arrow--down');
                  $('.drop-element').css('top', topForDrop).addClass('drop-arrow--up');
