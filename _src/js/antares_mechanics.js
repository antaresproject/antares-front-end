...
Antares.prototype.components = {
  ...
  divPreloader: function() {
    $.fn.divPreload = function(action) {
      ...
      if (action === 'on') {
        self.prepend(
          '<div class="md-preloader-container"><div class="md-preloader"><svg xmlns="http://www.w3.org/2000/svg" version="1.1" height="75" width="75" viewbox="0 0 75 75"><circle cx="37.5" cy="37.5" r="33.5" stroke-width="8"/></svg></div></div>',
        );
      } else if (action === 'off') {
        setTimeout(function() {
  ...
  settingsMenu: function(action) {
    ...
    $(
      document,
    ).on('click', '.menu-aside > li.has-submenu.submenu-open ul', function(e) {
      e.stopPropagation();
    });
    $(
      document,
    ).on('click', '.menu-aside > li.has-submenu.submenu-open', function(e) {
      e.stopPropagation();
      ...
      setTimeout(function() {
        ...
        $('.card--tabs').each(function(index, el) {
          var tabContainer = $(el).find('.mdl-tabs'),
            tabBar = tabContainer.find('.mdl-tabs__tab-bar'),
            tabBarWidth = tabContainer
              .find('.mdl-tabs__tab-bar')
              .outerWidth(true),
            singleTabsWidth = 0,
