...
Antares.prototype.swal = {
    ...
    init: function (theme, type, custom, confirmFunction) {
        ...
        $('.sweet-alert button').removeAttr('data-upgraded');
        //if exist
...
$(function () {
    window.APP = APP;
    APP.dialog.onLoad();
    antaresEvents.emit('performance.notifications');
});

export const AntaresNoty = {
    callNoty(typeNoty, textNoty, size, type, icon) {
        if (icon === undefined) {
            icon = '';
        }
        let timerNoty = 0,
            coef = 7,
            countLetters = textNoty.length;
        while (countLetters > coef) {
            timerNoty = timerNoty + 500;
            countLetters = countLetters - coef;
        }
        if (timerNoty < 4000) {
            timerNoty = 4000;
        }
        const typeOfCall = typeNoty + 'FM';
        if (typeNoty === 'success' || typeNoty === 'warning' || typeNoty === 'alert' || typeNoty === 'info') {
            window.noty(
                $.extend({}, APP.noti[typeOfCall](size, type, icon), {
                    text: textNoty,
                    timeout: timerNoty
                })
            );
...
window.AntaresNoty = AntaresNoty;
